...
class BoardViewController: UIViewController {
    ...
    @IBAction func showedNumber(sender : UIButton){
        ...
        if cards.isEmpty{
            self.progressBarDisplayer("Deixe-me pensar...", true)
    ...
    @IBAction func startToPlay(){
        ...
        collectionViewOutlet.card = selectCard()
        collectionViewOutlet.reloadData()
    }
    
    @IBAction func returnMenu(){
        let storyboard = UIStoryboard(name: "Main", bundle: nil)
        let board = storyboard.instantiateViewControllerWithIdentifier("ViewController") as UIViewController
        self.presentViewController(board, animated: true, completion: nil)
    }
    
    private func showResult(choosedNumber: Int){
        let myActionSheet : UIAlertController = UIAlertController(title: "Vamos ver se acertei... ", message: "Você pensou no número \(choosedNumber)!", preferredStyle: UIAlertControllerStyle.Alert)
        
        let myActionA = UIAlertAction(title: "Jogar Novamente", style: UIAlertActionStyle.Default, handler: { (ACTION:UIAlertAction!) in self.startToPlay() })
        
        let myActionB = UIAlertAction(title: "Sair", style: UIAlertActionStyle.Destructive, handler: { (ACTION:UIAlertAction!) in self.returnMenu() })
    ...
    private func selectCard() -> Card{
        if cards.isEmpty{
            return Card.init(keyValue: 0, cardNumbers: [""])
        }
        return cards.last!
    }
    ...
    private func progressBarDisplayer(msg:String, _ indicator:Bool ) {
        ...
        viewScrolled!.addSubview(messageFrame!)
    }
}